#' @rdname Shifted
#' @export
ShiftedExponential <- function(lambda, beta, shift=0) {
...
ShiftedExponentialRate <- S7::new_class(
  ...
  parent = ShiftedExponentialClass,
  properties = list(
    lambda = Parameter,
    shift = Parameter
  ),
...
S7::method(pdf_fn, ShiftedExponentialClass) <- function(distribution) function(x, rate, shift, log=FALSE) {
  stats::dexp(x - shift, rate, log)
}

S7::method(cdf_fn, ShiftedExponentialClass) <- function(distribution) function(q, rate, shift, lower.tail = TRUE, log.p = FALSE) {
  stats::pexp(q - shift, rate, lower.tail, log.p)
}

S7::method(qf_fn, ShiftedExponentialClass) <- function(distribution) function(p, rate, shift, lower.tail = TRUE, log.p = FALSE) {
  stats::qexp(p, rate, lower.tail, log.p) + shift
}

S7::method(rng_fn, ShiftedExponentialClass) <- function(distribution) function(n, rate, shift) {
